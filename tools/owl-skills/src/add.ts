...
import { track, trackHubInstall, setVersion, fetchAuditData } from "./telemetry.ts";
...
/**
 * Derive a package slug for Hub tracking from the parsed install source.
 * Returns null when the package cannot be determined (e.g. whole-repo install).
 */
function getPackageNameForTracking(parsed: ParsedSource): string | null {
  if (parsed.type === "hub-name" && parsed.name) {
    return parsed.name;
  }
  if (parsed.subpath) {
...
export async function runAdd(args: string[], options: AddOptions = {}): Promise<void> {
  ...
  if (!source) {
    ...
    console.log(
      `    ${pc.cyan("npx dt-skill add")} ${pc.yellow("<source>")} ${pc.dim("[options]")}`,
    );
    console.log();
    console.log(pc.dim("  Example:"));
    console.log(`    ${pc.cyan("npx dt-skill add")} ${pc.yellow("hello")}`);
    console.log();
    process.exit(1);
  }
  ...
  try {
    ...
    // Resolve hub names (e.g., "hello" → TreeTreeDi/skills-data) to GitHub shorthand
    if (parsed.type === "hub-name" && parsed.name) {
      parsed = resolveHubName(parsed.name);
    }
    ...
    if (sourceOwner === "openclaw" && !options.dangerouslyAcceptOpenclawRisks) {
      ...
      console.log();
      p.log.message(
        `If you understand the risks, re-run with:\n\n  ${pc.cyan(`npx dt-skill add ${source} --dangerously-accept-openclaw-risks`)}\n`,
      );
    ...
    // Report installs to Hub (fire-and-forget, never blocks)
    const packageNameForTracking = getPackageNameForTracking(parsed);
    if (packageNameForTracking && successful.length > 0) {
      const successfulSkillDisplayNames = new Set(successful.map((r) => r.skill));
      for (const skill of selectedSkills) {
        if (successfulSkillDisplayNames.has(getSkillDisplayName(skill))) {
          trackHubInstall(packageNameForTracking, skill.name);
        }
      }
    }
