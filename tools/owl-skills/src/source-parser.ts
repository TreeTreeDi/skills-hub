...
export function parseSource(input: string): ParsedSource {
  ...
  // Hub name: simple names (letters, digits, hyphens, dots, underscores) without slashes or colons
  // e.g., "hello", "code-review", "my.skill-pack"
  if (isHubName(input)) {
    return {
      type: "hub-name",
      url: input,
...
/**
 * Check if an input looks like a hub name (simple identifier without slashes/colons).
 * Hub names are simple identifiers like "hello", "code-review", "my.skill_pack".
 * They contain only letters, digits, hyphens, dots, and underscores — no slashes or colons.
 */
function isHubName(input: string): boolean {
  // Must not be empty
  if (!input) return false;
  // Must not contain slashes or colons (those indicate paths or URLs)
  if (input.includes("/") || input.includes(":")) return false;
  // Must not start with a dot (local paths like ./.hidden)
  if (input.startsWith(".")) return false;
  // Must match simple identifier pattern: letters, digits, hyphens, dots, underscores
  return /^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(input);
}

/**
 * Resolve a hub name to a GitHub shorthand ParsedSource using the config's defaultRepo.
 * e.g., "hello" → { type: "github", url: "https://github.com/TreeTreeDi/skills-data.git", subpath: "skills/hello" }
 */
export function resolveHubName(name: string): ParsedSource {
  const config = readConfig();
  return {
    type: "github",
    url: `https://github.com/${config.defaultRepo}.git`,
    subpath: `skills/${name}`,
  };
}
